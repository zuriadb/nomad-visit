...
class ThirdPage extends StatelessWidget {
  ...
  // универсальная кнопка входа — занимает всю доступную ширину внутри панели
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: LayoutBuilder(builder: (context, constraints) {
          ...
          // максимально желаемая ширина «панели» (как в макете) — 393
          // ignore: unused_local_variable
          final panelWidth = screenWidth >= 420 ? 393.0 : screenWidth * 0.92;

          return Column(
            children: [
              // Верхняя часть с приветствием — занимает ~40% высоты
              Expanded(
                flex: 4,
                child: Center(
                  child: Column(
              ...
              // Нижняя белая панель с закруглёнными углами — занимает оставшуюся высоту
            ...
            child: GestureDetector(
              onTap: canContinue
                  ? () {
                      ...
                      // сохраняем в DraftPlanProvider (merge)
                      Provider.of<DraftPlanProvider>(context, listen: false).merge(planData);

                      // идём дальше — FifthPage сама добавит свои поля в Draft
                      Navigator.push(
                        context,
                        MaterialPageRoute(
...
class FifthPage extends StatefulWidget {
  /// Оставляем callback опциональным — для совместимости с остальным кодом.
...
class _LoadingPageState extends State<LoadingPage> {
  ...
  void initState() {
    super.initState();
    // Симуляция задержки для "генерации плана"
...
class _MainPageState extends State<MainPage> {
  ...
  void initState() {
    ...
    // инициализируем страницы (замени на свои импорты)
    _pages.addAll([
      const HomePage(),
      const SocialPage(),
      const ChatPage(),
      const PlanPage(), // читает из PlanProvider
      const ProfilePage(),
    ]);
  }
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Padding(
        ...
        child: ElevatedButton(
          ...
          onPressed: () {
            // Формируем окончательный план, используя draft + подробности (plan)
            final finalPlan = <String, dynamic>{
              ...
              // добавь другие поля, которые собирался в draft (person, preferences и т.д.)
              ...draft,
            };
            ...
            // Переходим в MainPage и открываем вкладку Plan (индекс 3)
...
class _ChatPageState extends State<ChatPage> {
  ...
  void _sendMessage() {
    ...
    setState(() {
      ...
      messages.add({"isUser": false, "text": "Жақсы таңдау! 🌍"}); // заглушка
    });
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            const CircleAvatar(
              radius: 50,
              backgroundImage: AssetImage("assets/images/avatar.png"), // поставь свою картинку
            ),
